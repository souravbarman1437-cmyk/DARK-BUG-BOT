    ...
    const GlobalTechInc = makeWASocket({
     ...
     auth: {
      ...
      getMessage: async (key) => {
         ...
         return msg?.message || "𝐇𝐄𝐘𝐘 𝐃𝐀𝐑𝐊 𝐁𝐔𝐆 𝐈𝐒 𝐍𝐎𝐖 𝐎𝐍𝐋𝐈𝐍𝐄!!"
      },
   ...
   if (pairingCode && !GlobalTechInc.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
         ...
         if (!Object.keys(PHONENUMBER_MCC).some(v => phoneNumber.startsWith(v))) {
            console.log(chalk.bgBlack(chalk.redBright("Start with country code of your WhatsApp Number, Example : 916295169569")))
            process.exit(0)
         }
      } else {
         phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`Please type your WhatsApp number 😍\nFor example: 916295169569 : `)))
         phoneNumber = phoneNumber.replace(/[^0-9]/g, '')
    ...
    GlobalTechInc.ev.on('messages.upsert', async chatUpdate => {
        ...
        try {
            ...
            require("./DARKBug8")(GlobalTechInc, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
...
GlobalTechInc.ev.on("connection.update",async  (s) => {
        ...
        if (connection == "open") {
            ...
            console.log(chalk.yellow(`🌿DARK BUG Connected to => ` + JSON.stringify(GlobalTechInc.user, null, 2)))
			await delay(1999)
